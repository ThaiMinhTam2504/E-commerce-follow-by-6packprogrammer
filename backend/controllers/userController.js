...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
    ...
    const { name, email, password, avatar, phoneNumber, birthDate, address } = req.body;
    ...
    // Check if phoneNumber is a Vietnamese phone number
    const phoneRegex = /(03|05|07|08|09|01[2|6|8|9])+([0-9]{8})\b/;
    if (!phoneRegex.test(phoneNumber)) {
        return next(new ErrorHander("Phone number is not a valid Vietnamese phone number", 400));
    }

    if (!name || !email || !password || !avatar || !phoneNumber || !birthDate || !address) {
